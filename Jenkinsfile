node('built-in') 
{
    ...
    {
      sh 'scp var/jenkins_home/workspace/jenkins job/target/webapp.war ubuntu@172.31.7.160/opt/tomcat/webapps/qaenv.war' 
    }
    // replace IP address in continuous deploy/delivery with the private IP of the qa/prod server
    stage('Continuous Delivery') 
    {
      sh 'scp var/jenkins_home/workspace/jenkins job/target/webapp.war ubuntu@172.31.7.160/opt/tomcat/webapps/prodenv.war'
    }
